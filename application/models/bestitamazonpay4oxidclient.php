...
class bestitAmazonPay4OxidClient extends bestitAmazonPay4OxidContainer
{
    ...
    protected function _getAmazonClient($aConfig = array())
    {
        if ($this->_oAmazonClient === null) {
            ...
            $this->_oAmazonClient->setLogger($this->_oLogger);
        }
    ...
    {
        $this->_oLogger->debug('Try to add sandbox simulation params to request params', array('method' => $sMethod));
        ...
        if ((bool) $this->getConfig()->getConfigParam('blAmazonSandboxActive') !== true
            ...
            $this->_oLogger->debug('Simulation or sandbox not active');
            return;
        }
    ...
    public function processOrderReference(oxOrder $oOrder, stdClass $oOrderReference)
    {
        ...
        $this->_oLogger->debug('Process order reference', array('state' => $sOrderReferenceStatus));
        ...
        if ($sOrderReferenceStatus === 'Open'
            ...
            $this->_oLogger->debug('Reauthorize order');
    ...
    public function setOrderReferenceDetails($oBasket = null, array $aRequestParameters = array())
    {
        ...
        $this->_oLogger->debug('Set order reference details', array('params' => $aRequestParameters));
    ...
    public function confirmOrderReference(array $aRequestParameters = array())
    {
        ...
        $this->_oLogger->debug('Confirm order reference', array('reference' => $aRequestParameters['amazon_order_reference_id']));
    ...
    {
        $this->_oLogger->debug('Try to set transaction order error status', array('sStatus' => $sStatus));
        ...
        if (isset($oData->Error->Code) && (bool) $oData->Error->Code !== false) {
            ...
            $this->_oLogger->debug('Persist transaction order error status', array('sStatus' => $sStatus));
    ...
    public function setCaptureState(oxOrder $oOrder, stdClass $oCaptureDetails, $blOnlyNotEmpty = false)
    {
        ...
        if ($oCaptureDetails->CaptureStatus->State === 'Completed'
            ...
            $this->_oLogger->debug('Set paid date into order');
    ...
    {
        $this->_oLogger->debug('Try to save capture');
    ...
    public function refund($fPrice, $oOrder = null, $aRequestParameters = array())
    {
        ...
        $this->_oLogger->debug('Refund order', array('price' => $fPrice,'params' => $aRequestParameters));
        ...
        if ($oData && $oOrder !== null) {
            ...
            $this->_oLogger->debug('Persist refundData', array('query' => $sQuery));
    ...
    {
        $this->_oLogger->debug('Fetch amazon user info');
