...
# Resolve arguments, if no arguments are given use the ENV vars
if [[ $# -eq 2 ]]
  then
    OXID_SERIES=${1}
    OXID_VERSION=${2}
...
echo "=== Rewrite config ==="
if [[ ${OXID_SERIES} == 5 ]]; then
    sed -i 's|<dbHost_ce>|'${DB_HOST}'|; s|<dbName_ce>|oxidehop_ce|; s|<dbUser_ce>|'${DB_USER}'|; s|<dbPwd_ce>|'${DB_PASS}'|; s|<sShopURL_ce>|http://localhost:'${APACHE_PORT}/'|; s|<sSSLShopURL_ce>|https://localhost:'${APACHE_SSL_PORT}/'|; s|<sShopDir_ce>|'${SHOP_WEB_DIR}'|; s|<sCompileDir_ce>|'${SHOP_WEB_DIR}'/tmp|; s|<iUtfMode>|0|; s|$this->iDebug = 0|$this->iDebug = 1|; s|mysql|mysqli|' ${SHOP_WEB_DIR}/config.inc.php
else
    sed -i 's|<dbHost>|'${DB_HOST}'|; s|<dbName>|oxidehop_ce|; s|<dbUser>|'${DB_USER}'|; s|<dbPwd>|'${DB_PASS}'|; s|<sShopURL>|http://localhost:'${APACHE_PORT}/'|; s|<sSSLShopURL>|https://localhost:'${APACHE_SSL_PORT}/'|; s|<sShopDir>|'${SHOP_WEB_DIR}'|; s|<sCompileDir>|'${SHOP_WEB_DIR}'/tmp|; s|$this->iDebug = 0|$this->iDebug = 0|' ${SHOP_WEB_DIR}/config.inc.php
fi
